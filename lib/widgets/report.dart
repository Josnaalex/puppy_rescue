...
class _LocationAndImageFormState extends State<LocationAndImageForm> {
  ...
  Future<void> _fetchData() async {
    ...
    final shelterNames = data.map<String>((e) => e['location'].toString()).toList();
    final shelterIds = Map.fromIterables(
      shelterNames,
  ...
  final report = {
    'location': _loc,
    'landmark': _landmark,
    // 'shelter_id': shelterIds,
    'image_url': _image?.path,
  };

  await supabase.from('reports').insert(report).execute();
}
        ...
        child: Padding(
          ...
          child: Column(
           ...
           DropdownButton<String>(
                  value: _selectedShelter,
                  onChanged: (String? newValue) => setState(() {_selectedShelter = newValue;}),
                  items: animalShelters.map<DropdownMenuItem<String>>((shelter) { 
